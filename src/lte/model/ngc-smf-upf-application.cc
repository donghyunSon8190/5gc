...
NgcSmfUpfApplication::NgcSmfUpfApplication (const Ptr<VirtualNetDevice> tunDevice, const Ptr<Socket> s1uSocket)
    ...
    m_s11SapMme (0)
{
  ...
  m_s1uSocket->SetRecvCallback (MakeCallback (&NgcSmfUpfApplication::RecvFromS1uSocket, this));
  m_s11SapSmf = new MemberNgcS11SapSmf<NgcSmfUpfApplication> (this);
}
...
void 
NgcSmfUpfApplication::RecvFromS1uSocket (Ptr<Socket> socket)
{
...
void 
NgcSmfUpfApplication::SendToS1uSocket (Ptr<Packet> packet, Ipv4Address enbAddr, uint32_t teid)
{
...
void 
NgcSmfUpfApplication::SetS11SapMme (NgcS11SapMme * s)
{
  m_s11SapMme = s;
}

NgcS11SapSmf* 
NgcSmfUpfApplication::GetS11SapSmf ()
{
  return m_s11SapSmf;
}
...
void 
NgcSmfUpfApplication::DoCreateSessionRequest (NgcS11SapSmf::CreateSessionRequestMessage req)
{
  ...
  NgcS11SapMme::CreateSessionResponseMessage res;
  res.teid = req.imsi; // trick to avoid the need for allocating TEIDs on the S11 interface

  for (std::list<NgcS11SapSmf::BearerContextToBeCreated>::iterator bit = req.bearerContextsToBeCreated.begin ();
       ...
       ++bit)
    {
      ...
      NgcS11SapMme::BearerContextCreated bearerContext;
...
void 
NgcSmfUpfApplication::DoModifyBearerRequest (NgcS11SapSmf::ModifyBearerRequestMessage req)
{
  ...
  NgcS11SapMme::ModifyBearerResponseMessage res;
  res.teid = imsi; // trick to avoid the need for allocating TEIDs on the S11 interface
  res.cause = NgcS11SapMme::ModifyBearerResponseMessage::REQUEST_ACCEPTED;
  m_s11SapMme->ModifyBearerResponse (res);
}
 
void
NgcSmfUpfApplication::DoDeleteBearerCommand (NgcS11SapSmf::DeleteBearerCommandMessage req)
{
  NS_LOG_FUNCTION (this << req.teid);
  uint64_t imsi = req.teid; // trick to avoid the need for allocating TEIDs on the S11 interface
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  ...
  NgcS11SapMme::DeleteBearerRequestMessage res;
  res.teid = imsi;

  for (std::list<NgcS11SapSmf::BearerContextToBeRemoved>::iterator bit = req.bearerContextsToBeRemoved.begin ();
       bit != req.bearerContextsToBeRemoved.end ();
       ++bit)
    {
      NgcS11SapMme::BearerContextRemoved bearerContext;
      bearerContext.epsBearerId =  bit->epsBearerId;
...
void
NgcSmfUpfApplication::DoDeleteBearerResponse (NgcS11SapSmf::DeleteBearerResponseMessage req)
{
  NS_LOG_FUNCTION (this << req.teid);
  uint64_t imsi = req.teid; // trick to avoid the need for allocating TEIDs on the S11 interface
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  ...
  for (std::list<NgcS11SapSmf::BearerContextRemovedSmfUpf>::iterator bit = req.bearerContextsRemoved.begin ();
