...
NgcEnbApplication::NgcEnbApplication (Ptr<Socket> lteSocket, Ptr<Socket> s1uSocket, Ipv4Address enbS1uAddress, Ipv4Address smfS1uAddress, uint16_t cellId)
    ...
    m_cellId (cellId)
{
  ...
  m_s1uSocket->SetRecvCallback (MakeCallback (&NgcEnbApplication::RecvFromS1uSocket, this));
  m_lteSocket->SetRecvCallback (MakeCallback (&NgcEnbApplication::RecvFromLteSocket, this));
  m_s1SapProvider = new MemberNgcEnbS1SapProvider<NgcEnbApplication> (this);
  m_s1apSapEnb = new MemberNgcS1apSapEnb<NgcEnbApplication> (this);
}
...
void 
NgcEnbApplication::SetS1SapUser (NgcEnbS1SapUser * s)
{
  m_s1SapUser = s;
}
...
NgcEnbS1SapProvider* 
NgcEnbApplication::GetS1SapProvider ()
{
  return m_s1SapProvider;
}

void 
NgcEnbApplication::SetS1apSapMme (NgcS1apSapEnbProvider * s)
{
...
/* jhlim */
void
NgcEnbApplication::SetS1apSapMme (NgcS1apSapMme * s)
{
...
NgcS1apSapEnb* 
NgcEnbApplication::GetS1apSapEnb ()
{
  return m_s1apSapEnb;
}
...
void 
NgcEnbApplication::DoPathSwitchRequest (NgcEnbS1SapProvider::PathSwitchRequestParameters params)
{
  NS_LOG_FUNCTION (this);
  ...
  std::list<NgcS1apSapMme::ErabSwitchedInDownlinkItem> erabToBeSwitchedInDownlinkList;
  for (std::list<NgcEnbS1SapProvider::BearerToBeSwitched>::iterator bit = params.bearersToBeSwitched.begin ();
       ...
       ++bit)
    {
      ...
      NgcS1apSapMme::ErabSwitchedInDownlinkItem erab;
...
/*
void 
NgcEnbApplication::DoPathSwitchRequest (NgcEnbS1SapProvider::PathSwitchRequestParameters params, int dummy)
{
  NS_LOG_FUNCTION (this);
  uint16_t enbUeS1Id = params.rnti;  
  uint64_t mmeUeS1Id = params.mmeUeS1Id;
...
void 
NgcEnbApplication::DoInitialContextSetupRequest (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, std::list<NgcS1apSapEnb::ErabToBeSetupItem> erabToBeSetupList)
{ //sjkang1021
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("In EnpEnbApplication DoInitialContextSetupRequest size of the erabToBeSetupList is " << erabToBeSetupList.size());

  for (std::list<NgcS1apSapEnb::ErabToBeSetupItem>::iterator erabIt = erabToBeSetupList.begin ();
       ...
       ++erabIt)
    {
      ...
      struct NgcEnbS1SapUser::DataRadioBearerSetupRequestParameters params;
      params.rnti = rnti;
...
void 
NgcEnbApplication::DoPathSwitchRequestAcknowledge (uint64_t enbUeS1Id, uint64_t mmeUeS1Id, uint16_t gci, std::list<NgcS1apSapEnb::ErabSwitchedInUplinkItem> erabToBeSwitchedInUplinkList)
{
  NS_LOG_FUNCTION (this);
  ...
  NS_ASSERT_MSG (imsiIt != m_imsiRntiMap.end (), "unknown IMSI");
  uint16_t rnti = imsiIt->second;
  NgcEnbS1SapUser::PathSwitchRequestAcknowledgeParameters params;
  params.rnti = rnti;
  m_s1SapUser->PathSwitchRequestAcknowledge (params);
}

void 
NgcEnbApplication::RecvFromLteSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this);  
...
void 
NgcEnbApplication::RecvFromS1uSocket (Ptr<Socket> socket)
{
...
void 
NgcEnbApplication::SendToLteSocket (Ptr<Packet> packet, uint16_t rnti, uint8_t bid)
{
...
void 
NgcEnbApplication::SendToS1uSocket (Ptr<Packet> packet, uint32_t teid)
{
...
NgcEnbApplication::DoReleaseIndication (uint64_t imsi, uint16_t rnti, uint8_t bearerId)
{
  ...
  std::list<NgcS1apSapMme::ErabToBeReleasedIndication> erabToBeReleaseIndication;
  NgcS1apSapMme::ErabToBeReleasedIndication erab;
