...
NgcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  if (procedureCode == NgcX2Header::HandoverPreparation)
    {
      ...
      else if (messageType == NgcX2Header::SuccesfulOutcomToLte)
             {
  ...
  else if (procedureCode == NgcX2Header::NotifyMmWaveLteHandover)
    {
...
void
NgcX2::DoSendHandoverRequestAckToLte (NgcX2SapProvider::HandoverRequestAckParams params)
{
  NS_LOG_FUNCTION (this);
 //std::cout << "-------------- X2 will send handover request ack to Lte eNB -----------" <<std::endl;
  ...
  NgcX2Header x2Header;
  x2Header.SetMessageType (NgcX2Header::SuccesfulOutcomToLte);
  x2Header.SetProcedureCode (NgcX2Header::HandoverPreparation);
  x2Header.SetLengthOfIes (x2HoAckHeader.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2HoAckHeader.GetNumberOfIes ());
...
void
NgcX2::DoNotifyLteMmWaveHandoverCompleted (NgcX2SapProvider::SecondaryHandoverParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  NgcX2Header x2Header;
  x2Header.SetMessageType (NgcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (NgcX2Header::NotifyMmWaveLteHandover);
  x2Header.SetLengthOfIes (x2mcHeader.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2mcHeader.GetNumberOfIes ());
