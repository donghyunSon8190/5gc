...
NS_LOG_COMPONENT_DEFINE ("NgcTestS1uDownlink");
...
class NgcS1uDlTestCase : public TestCase
{
public:
  NgcS1uDlTestCase (std::string name, std::vector<EnbDlTestData> v);
  virtual ~NgcS1uDlTestCase ();
...
NgcS1uDlTestCase::NgcS1uDlTestCase (std::string name, std::vector<EnbDlTestData> v)
  : TestCase (name),
    m_enbDlTestData (v)
{
}

NgcS1uDlTestCase::~NgcS1uDlTestCase ()
{
}

void 
NgcS1uDlTestCase::DoRun ()
{
  ...
  for (std::vector<EnbDlTestData>::iterator enbit = m_enbDlTestData.begin ();
       ...
       ++enbit)
    {
      ...
      // we test NGC without LTE, hence we use:
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          uint64_t imsi = u+1;
          ngcHelper->AddUe (ueLteDevice, imsi);
          ngcHelper->ActivateEpsBearer (ueLteDevice, imsi, NgcTft::Default (), EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT));
 ...
 */
class NgcS1uDlTestSuite : public TestSuite
{
public:
  NgcS1uDlTestSuite ();
  
} g_ngcS1uDlTestSuiteInstance;

NgcS1uDlTestSuite::NgcS1uDlTestSuite ()
  : TestSuite ("ngc-s1u-downlink", SYSTEM)
{  
  ...
  e1.ues.push_back (f1);
  v1.push_back (e1);
  AddTestCase (new NgcS1uDlTestCase ("1 eNB, 1UE", v1), TestCase::QUICK);
  ...
  v2.push_back (e2);
  AddTestCase (new NgcS1uDlTestCase ("1 eNB, 2UEs", v2), TestCase::QUICK);


  std::vector<EnbDlTestData> v3;  
  v3.push_back (e1);
  v3.push_back (e2);
  AddTestCase (new NgcS1uDlTestCase ("2 eNBs", v3), TestCase::QUICK);
  ...
  v4.push_back (e1);
  v4.push_back (e2);
  AddTestCase (new NgcS1uDlTestCase ("3 eNBs", v4), TestCase::QUICK);
  ...
  e5.ues.push_back (f5);
  v5.push_back (e5);
  AddTestCase (new NgcS1uDlTestCase ("1 eNB, 10 pkts 3000 bytes each", v5), TestCase::QUICK);
  ...
  e6.ues.push_back (f6);
  v6.push_back (e6);
  AddTestCase (new NgcS1uDlTestCase ("1 eNB, 50 pkts 3000 bytes each", v6), TestCase::QUICK);
  ...
  e7.ues.push_back (f7);
  v7.push_back (e7);
  AddTestCase (new NgcS1uDlTestCase ("1 eNB, 10 pkts 15000 bytes each", v7), TestCase::QUICK);
  ...
  e8.ues.push_back (f8);
  v8.push_back (e8);
  AddTestCase (new NgcS1uDlTestCase ("1 eNB, 100 pkts 15000 bytes each", v8), TestCase::QUICK);
}
