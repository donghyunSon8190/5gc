...
/**
 * Sample simulation script for LTE+NGC. It instantiates several eNodeB,
 * attaches one UE per eNodeB starts a flow for each UE to  and from a remote host.
 * It also  starts yet another flow between each UE pair.
 */
NS_LOG_COMPONENT_DEFINE ("MmWaveNgcTdma");
int
main (int argc, char *argv[])
{
	//LogComponentEnable ("LteUeRrc", LOG_LEVEL_ALL);
	//LogComponentEnable ("LteEnbRrc", LOG_LEVEL_ALL);
	//	LogComponentEnable("mmWavePointToPointNgcHelper",LOG_LEVEL_ALL);
	//	LogComponentEnable("NgcUeNas",LOG_LEVEL_ALL);
...
//	LogComponentEnable ("LteRlcAm", LOG_LEVEL_LOGIC);
	...
	ueIpIface = ngcHelper->AssignUeIpv4Address (NetDeviceContainer (uemmWaveDevs));
...
//		}
	...
	//p2ph.EnablePcapAll("mmwave-ngc-simple");
