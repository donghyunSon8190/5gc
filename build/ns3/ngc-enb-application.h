...
class NgcEnbApplication : public Application
{
  ...
  friend class MemberNgcEnbS1SapProvider<NgcEnbApplication>;
  friend class MemberNgcS1apSapEnb<NgcEnbApplication>;
...
public:
   ...
   */
  NgcEnbApplication (Ptr<Socket> lteSocket, Ptr<Socket> s1uSocket, Ipv4Address enbS1uAddress, Ipv4Address smfS1uAddress, uint16_t cellId);
   ...
   */
  void SetS1SapUser (NgcEnbS1SapUser * s);

  /** 
   * 
   * \return the S1 SAP Provider
   */
  NgcEnbS1SapProvider* GetS1SapProvider ();
   ...
   */
  void SetS1apSapMme (NgcS1apSapEnbProvider * s);

  /* jhlim */
  void SetS1apSapMme (NgcS1apSapMme * s);
  /** 
   * 
   * \return the ENB side of the S1-AP SAP 
   */
  NgcS1apSapEnb* GetS1apSapEnb ();
...
private:
  ...
  void DoPathSwitchRequest (NgcEnbS1SapProvider::PathSwitchRequestParameters params);
  void DoUeContextRelease (uint16_t rnti);

  /* jhlim */
  void DoInitialUeMessage (uint64_t imsi, uint16_t rnti, int dummy);
  void DoPathSwitchRequest (NgcEnbS1SapProvider::PathSwitchRequestParameters params, int dummy);

  // S1-AP SAP ENB methods
  void DoInitialContextSetupRequest (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, std::list<NgcS1apSapEnb::ErabToBeSetupItem> erabToBeSetupList);
  void DoPathSwitchRequestAcknowledge (uint64_t enbUeS1Id, uint64_t mmeUeS1Id, uint16_t cgi, std::list<NgcS1apSapEnb::ErabSwitchedInUplinkItem> erabToBeSwitchedInUplinkList);
   ...
   * 
   */
  NgcS1apSapEnbProvider* m_s1apSapEnbProvider;

  /**
   * ENB side of the S1-AP SAP eNB endpoint
   * 
   */
  NgcS1apSapEnb* m_s1apSapEnb;
